<script>
	...
	onMount(async () => {
		try {
			const response = await fetch('http://192.168.0.91:8080/main')
	...
	async function songsforalbum(albumid) {
		try {
			const response = await fetch('http://192.168.0.91:8080/songsforalbum/' + albumid)
				.then((response) => response.json())
				.then((data) => {
					sfaData = data;
				});
	...
	async function getCurrentPlayingImg(albid) {
		try {
			const response = await fetch('http://192.168.0.91:8080/currentPlayingImg/' + albid)
				.then((response) => response.json())
				.then((data) => {
	...
	async function addSongToPlaylist(songid) {
		const u = 'http://192.168.0.91:8080/addsongtoplaylist';
